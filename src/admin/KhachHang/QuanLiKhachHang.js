...
=======
  const fetchCustomer = (trimmedPhoneNumber) => {
    setLoadingCustomer(true);
    setCustomer(null); // Xóa dữ liệu khách hàng cũ
    setAllCustomers(null); // Xóa dữ liệu tất cả khách hàng cũ
    setErrorMessage(""); // Reset thông báo lỗi
    ...
    if (trimmedPhoneNumber) {
      // Truy vấn một khách hàng theo số điện thoại từ node `user`
      const customerRef = ref(database, `user/${trimmedPhoneNumber}`);
      onValue(
        customerRef,
        (snapshot) => {
          const data = snapshot.val();
          if (data) {
            setCustomer({ phoneNumber: trimmedPhoneNumber, ...data });
          } else {
            setErrorMessage(
              "Không có thông tin khách hàng với số điện thoại này."
            );
          }
          setLoadingCustomer(false);
        },
...
=======
    ...
    // Nếu hợp lệ, tiến hành tìm kiếm
    setErrorMessage("");
    fetchCustomer(trimmedPhoneNumber);
>>>>>>> b08c117 (Cập nhật dự án lên GitHub)
  };
