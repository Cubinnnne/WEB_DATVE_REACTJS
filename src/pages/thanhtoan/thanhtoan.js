...
import axios from "axios";
import { useLocation, useNavigate } from "react-router-dom";
import GooglePayComponent from "../../Payment/GooglePay/GooglePayComponent";
import "./thanhtoan.css";

function ThanhToan() {
  const location = useLocation();
  const navigate = useNavigate();
  const { tripInfo, customerInfo, pickupDropoff, totalAmount } =
    location.state || {};
  ...
  const paymentMethods = [
    ...
    {
      name: "GooglePay",
      icon: "https://pay.google.com/gp/promo/p_referrals/static/images/gp3-lockup.svg",
    },
  ];
  ...
  const handleConfirmPayment = async () => {
    try {
      if (selectedPaymentMethod === "ZaloPay") {
        ...
        const paymentLink =
          response?.data?.paymentUrl || response?.data?.order_Url;
        ...
        if (paymentLink) {
          ...
          navigate("/ketquathanhtoan", {
            state: {
              transactionId:
                response.data.app_trans_id || response.data.orderId || null,
              tripInfo: tripInfo || {},
              customerInfo: customerInfo || {},
              pickupDropoff: pickupDropoff || {},
              totalAmount: totalAmount || 0,
  ...
  return (
    <div className="payment-container">
      <div className="payment-methods-section">
        <h2>Chọn phương thức thanh toán</h2>
        {paymentMethods.map((method) => (
          <div key={method.name} className={`payment-option ${method.name}`}>
            <input
              type="radio"
              id={method.name}
              name="paymentMethod"
      ...
      <div className="trip-info info-details">
        <h3>Thông tin hành khách</h3>
        <p>
          Họ và tên: <br /> <span>{customerInfo?.name || ""}</span>
        </p>
        <p>
          Số điện thoại: <br /> <span>{customerInfo?.phone || ""}</span>
        </p>
        <p>
          Email: <br /> <span>{customerInfo?.email || ""}</span>
        </p>
      </div>
      ...
      <div className="price-info info-details">
        ...
        <p>
          Giá vé lượt đi:{" "}
          <span>
            {totalAmount ? `${formatPriceToString(totalAmount)}.000đ` : ""}
          </span>
        </p>
        <p>
          Phí thanh toán: <span>0đ</span>
        </p>
      ...
      <div className="trip-info info-details">
        ...
        <p>
          Tuyến xe: <br />{" "}
          <span>{tripInfo ? `${tripInfo.from} - ${tripInfo.to}` : ""}</span>
        </p>
        <p>
          Giờ xuất bến: <span>{tripInfo?.hour || ""}</span>
        </p>
        <p>
          Số lượng ghế: <span>{tripInfo?.seatCount || ""}</span>
        </p>
        <p>
          Mã ghế đặt: <br /> <span>{tripInfo?.seats?.join(", ") || ""}</span>
        </p>
        <p>
          Điểm đón: <br /> <span>{pickupDropoff?.pickup || ""}</span>
        </p>
        <p>
          Điểm trả: <br /> <span>{pickupDropoff?.dropoff || ""}</span>
        </p>
      </div>
    </div>
  );
